...
const velocityClass = (direction: DriftData['trajectory']['velocity']['direction']): string => {
  ...
  return 'text-ink-secondary bg-surface-ui/60 border-edge';
};
  ...
  return (
    <div className="space-y-6">
      <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
        <div className="rounded-xl border border-edge bg-surface-subtle/40 p-4 md:col-span-1">
          <p className="text-xs uppercase tracking-wide text-ink-faint">Composite Drift</p>
          <div className="mt-2 flex items-end gap-2">
            <p className={`text-4xl font-bold ${scoreClass(data.composite.score)}`}>{data.composite.score}</p>
            <p className="text-sm text-ink-faint mb-1">/100</p>
          </div>
          <p className="mt-2 text-xs text-ink-faint">Higher score means architecture is further from declared intent.</p>
        </div>

        <div className="rounded-xl border border-edge bg-surface-subtle/40 p-4 md:col-span-2">
          <div className="flex items-center justify-between gap-3">
            <p className="text-xs uppercase tracking-wide text-ink-faint">Drift Velocity</p>
      ...
      <div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
          {(Object.keys(data.components) as Array<keyof DriftData['components']>).map((key) => {
            ...
            return (
              <div key={key} className="rounded-lg border border-edge bg-surface-subtle/40 p-3">
                <div className="flex items-center justify-between">
                  <p className="text-xs uppercase tracking-wide text-ink-faint">{componentLabel[key]}</p>
                ...
                <p className="mt-2 text-[11px] text-ink-faint">{component.violationCount} signals</p>
              </div>
            );
          })}
        </div>
      ...
      <div>
        <h3 className="text-sm font-medium text-ink-secondary mb-3">Drift Trajectory</h3>
        <div className="h-48 rounded-xl border border-edge bg-surface-subtle/30 p-4">
          {trajectory.length > 0 ? (
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-4">
        <div className="rounded-xl border border-edge bg-surface-subtle/30 p-4">
          ...
          <div className="space-y-2">
            {sortedLayers.length > 0 ? sortedLayers.map((layer) => (
              ...
              <p className="text-xs text-ink-faint">No architecture layers configured.</p>
            )}
          </div>
        </div>
